...
void CMisc::AutoStrafe(CUserCmd* pCmd)
{
	...
	if (!pLocal || pLocal->GetMoveType() == 8)
		return;

	static bool lastHeldJump = pCmd->buttons & CUserCmd::Button_Jump;
	if (~pCmd->buttons & CUserCmd::Button_Jump && !lastHeldJump)
		return;
	...
	if (pDelta)
	{
		const float yaw = Helpers::degreesToRadians(pCmd->viewangles.y);
		const float velDir = std::atan2f(pLocal->GetVelocity().y, pLocal->GetVelocity().x) - yaw;
		const float wishAng = std::atan2f(-pCmd->sidemove, pCmd->forwardmove);
		const float delta = Helpers::angleDiffRad(velDir, wishAng);

		float moveDir = delta < 0.0f ? velDir + pDelta : velDir - pDelta;

		pCmd->forwardmove = std::cosf(moveDir) * 450.0f;
		pCmd->sidemove = -std::sinf(moveDir) * 450.0f;
	}
		lastHeldJump = pCmd->buttons & CUserCmd::Button_Jump;
